...
<xsl:stylesheet
	...
	<xsl:template name="add-remove-button">
		...
		<xsl:if test="(number($min-occurs) or number($max-occurs) or $max-occurs = 'unbounded') and not($min-occurs = $max-occurs) and not($min-occurs = '1' and $max-occurs = '') and not($max-occurs = '1' and $min-occurs = '')">
